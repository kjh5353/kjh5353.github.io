...
function collectTimetableData() {
  ...
  document.querySelectorAll('.timetable-row').forEach(row => {
    ...
    const val = row.querySelector('.timetable-input').value;
    if (val) tt[time] = val;
  });
  return tt;
}
...
function renderTimetable(timetable) {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    const val = timetable[hourStr] || '';
    
    const row = document.createElement('div');
    row.className = 'timetable-row';
    row.dataset.time = hourStr;
    row.innerHTML = `
      <div class="timetable-time">${hourStr}</div>
      <input type="text" class="timetable-input" value="${val}" placeholder="일정 입력...">
    `;
    row.querySelector('.timetable-input').addEventListener('input', scheduleSave);
    container.appendChild(row);
  }
  // 현재 시스템 시간에 맞춰 중앙으로 스크롤 및 현재 시간 표시
  setTimeout(() => {
    const now = new Date();
    const currentHour = String(now.getHours()).padStart(2, '0') + ':00';
    const row = container.querySelector(`[data-time="${currentHour}"]`);
    if (row) {
      row.classList.add('is-current-hour');
      const scrollContainer = container.parentElement;
      const targetScroll = row.offsetTop - (scrollContainer.clientHeight / 2) + (row.clientHeight / 2);
